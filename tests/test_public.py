...
class PlannerTests(unittest.TestCase):
    ...
    def test_eligible(self):
        ...
        self.assertEqual(result["price_cents"], 1)
    ...
    def test_insufficient_funding(self):
        self.assertEqual(self.changed(balance_cents=10)["reason"], "INSUFFICIENT_SCOPED_FUNDS")

    def test_exact_funding_boundary(self):
        self.assertEqual(self.changed(balance_cents=11)["status"], "PLAN")

    def test_no_double_position(self):
        self.assertEqual(self.changed(position_contracts=1)["reason"], "EXISTING_EXPOSURE")

    def test_no_double_open_order(self):
        self.assertEqual(self.changed(open_contracts=10)["reason"], "EXISTING_EXPOSURE")

    def test_final_crossing_check(self):
        self.assertEqual(self.changed(best_ask_cents=1)["reason"], "POST_ONLY_WOULD_CROSS")

    def test_price_grid(self):
        self.assertEqual(self.changed(one_cent_supported=False)["status"], "HOLD")

    def test_invalid_price(self):
        self.assertEqual(self.changed(best_ask_cents=100)["status"], "INVALID")
...
class PackageTests(unittest.TestCase):
    ...
    def test_launchers_preserve_local_root_and_isolation(self):
        ...
        self.assertEqual(launchers, sorted(["Kalshi10x1cPublic.bat", "Kalshi10x1cPublic_Export.bat"]))
